...
\begin{itemize}
    ...
    specificato nel \underbar{\hyperref[legalpassword]{RNF \ref*{legalpassword}}}.
\end{itemize}
...
\begin{itemize}
    ...
    \item L'inserimento (in una pagina dedicata) di una nuova password che,
    come in fase di registrazione, deve essere scritta, confermata ed essere
    conforme al RNF \ref{legalpassword}.
\end{itemize}
\end{funcreq}
...
\label{logout}
\textbf{Logout }
L'utente registrato e autenticato deve poter interrompere la sessione di accesso
al servizio. Questa procedura di \textit{logout} realizza il passaggio dell'utente
dallo stato \textit{autenticato} a quello \textit{anonimo}.
\end{funcreq}
...
essere possibile:
\begin{enumerate}
    \item Visualizzare tale catalogo in una vista dedicata. Il catalogo mostra
    i campi di ogni problema, descritti nel RNF \ref{metadata}.
...
deve rendere disponibile alla vista dell'utente i dati relativi al
problema e specificati nel RNF \ref{formatoproblema}.
\end{funcreq}
...
a risolverlo. La sessione di esercitazione avviene in una pagina apposita, dove
l'utente deve poter disporre di tutti gli strumenti necessari alla realizzazione
delle seguenti funzionalità:
\begin{enumerate}
    \item Visualizzare il contenuto dell'esercizio (RNF \ref{formatoproblema}).
    
    \item Visualizzare i test cases (RF \ref{test}.2).
...
\begin{enumerate}
    ...
    \item L'utente deve essere in grado di essere notificato se il codice supera
    tutti i test case oppure se almeno un
    test case non è stato superato. In entrambi i casi l'utente verrà avvisato.
    Per ogni test case, sia l'input che l'output
    desiderato sono visibili.
    ...
    test case disponibili, la sessione di esercitazione termina: deve
    essere visualizzato il tempo impiegato e, qualora l'utente sia autenticato,
    il problema deve essere contrassegnato come \textit{risolto} nel campo
    \textit{stato} del problema e i progressi devono essere aggiornati.
\end{enumerate}
\end{funcreq}
...
possibile soluzione. Il video è accessibile sia dal catalogo che dalla
finestra dedicata allo svolgimento del problema.
\end{funcreq}
...
\begin{nonfuncreq}
\textbf{Prestazioni }
%La “User experience” deve essere ai livelli dei moderni siti web, se l’utente si trova in una
%condizione di connettività normale (4G - WI-FI).
[decommenta]
\end{nonfuncreq}
...
L'invio dei messaggi di posta elettronica, specificati nei requis, sarà gestito
