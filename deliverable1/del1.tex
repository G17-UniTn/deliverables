...
\fancyhead[C]{\footnotesize\textit{Documento:} D1 \hfill SleepCode \hfill \textit{Versione:} 2.4}
...
\begin{enumerate}
    ...
    \item Utilizzare un cronometro (\textcolor{blue}{\underbar{\hyperref[chrono]{RF \ref*{chrono}}}}).
...
\begin{enumerate}
    ...
    correttezza risolutiva dell'algoritmo—l'output fornito non è corretto.
\end{enumerate}
\end{funcreq}
...
\begin{funcreq}
    ...
    cronometro. Il cronometro è disponibile durante l'esercitazione, dopo aver scelto
...
In questo e nei documenti successivi, indichiamo con il termine \textit{profilo}
tutti i dati relativi all'attività di un utente registrato e/o autenticato sulla
piattaforma. Sono dunque inclusi l'\textit{account} (\textcolor{blue}{\underbar{\hyperref[signup]{RF \ref*{signup}}}}) e i \textit{progressi}
(\textcolor{blue}{\underbar{\hyperref[stats]{RF \ref*{stats}}}}).
\\
...
\begin{funcreq}
    ...
    (il mio like si vede nel catalogo, dal catalogo non posso mettere like,
    entrando nel problema vedo il numero di like)
\end{funcreq}

\begin{funcreq}
\textbf{Codice } Il codice scritto dall'utente autenticato, all'interno della
console, nell'area di esercitazione, viene salvato sul browser in locale, in
modo che l'utente, rivisitando i diversi esercizi, continuare l'esercizio.
\end{funcreq}
...
email e password:
\begin{enumerate}
\item L'utente deve poter eliminare il proprio account, e tutti i dati ad
esso collegati.
...
\begin{enumerate}
    \item Colore
    \item Layout dinamico: all'interno della pagina di esercitazione, divisa
    in console, riquadro dei test case e visualizzatore del contenuto del
    problema, l'utente deve essere in grado di modificare le dimensioni dei
    diversi riquadri a suo piacimento, comunque senza impattare l'usabilità
    della pagina di esercitazione del sito.

    \item Durante l'attesa di risposta del serve per qualsiasi richiesta
    ad esso fatta, l'utente deve essere essere notificato dell'avvenuto
    ricevimento della richiesta e dell'attesa conseguente, tramite opportuni
...
\begin{itemize}
    ...
    tra quelli specificati dalla libreria Yup,
    %\begin{center}
    %    \verb|! ? # $ % & @ * + . , ; : / - = _ \ ( ) [ ] { }|
    %\end{center}
\end{itemize}
