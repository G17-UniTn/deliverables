...
notificato di tale errore; è comunque possibile ritentare il login un
...
al servizio effettuando un logout, tramite l'interazione con un apposito pulsante.
Effettuato il logout, l'utente cessa di essere autenticato.
\end{funcreq}
...
problema desiderato. Dopodichè dovrà essere portato ad una pagina apposita, dove
l'utente potrà vedere il testo dell'esercizio, i test case per la verifica della correttezza dell'algoritmo
come descritto nel RF \ref{sintax}, una console dove scrivere il codice per risolvere il problema selezionato, e testarlo,
e appositi selettori per il linguaggio di programmazione desiderato. A tal fine, l'utente deve poter:
\begin{enumerate}
    \item Avviare e fermare a proprio piacimento un timer apposito per cronometrare il tempo speso
    ...
    scrittura deve poter essere effettuata in una vista apposita.
\end{enumerate}
\end{funcreq}
...
\begin{enumerate}
    ...
    \item L'utente deve essere in grado di essere notificato se il codice supera tutti i test case oppure se almeno un
    test case non è stato superato. In entrambi i casi l'utente verrà avvisato. Per ogni test case, sia l'input che l'output
    desiderato sono visibili.
    Inoltre, l'utente deve poter riscrivere e perfezionare l'algoritmo e sottoporre ripetutamente il codice ai test cases.

    \item Nel caso in cui l'utente autenticato sia in grado di superare tutti i test case disponibili, il problema dovrà
...
browser attualmente disponibili in commercio (Chrome, Edge, Firefox).
\end{nonfuncreq}
...
L'invio dei messaggi di posta elettronica, specificati nei re..., sarà gestito
